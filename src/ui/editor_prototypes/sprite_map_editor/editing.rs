...
impl UiSpriteMapEditor {
    ...
    pub(super) fn handle_edition_dragging(
        ...
        if snap_to_grid {
            let scale_pp = self.scale / self.pixels_per_point;
            let bounds_scaling = self.zoom / self.pixels_per_point;
            let bounds = self.selection_bounds.expect("unset even though some tiles are selected");

            let bounds_min_at_grid =
                point_to_tile_coords((bounds.min.to_vec2() * bounds_scaling).to_pos2(), scale_pp, self.zoom);
            let started_tile =
            ...
            let bounds_at_grid_exact_offset = bounds_min_at_grid * scale_pp * self.zoom;
            let started_tile_exact_offset = started_tile * scale_pp * self.zoom;
            let hovered_tile_exact_offset = hovered_tile * scale_pp * self.zoom;

            let bounds_offset = (bounds.min.to_vec2() * bounds_scaling) - bounds_at_grid_exact_offset;
